...
- Axiom01 component partials (`templates/components`)
- Bundled `dist/css/axiom01.min.css` and `dist/js/axiom01.min.js`
...
- Theme always loads bundled `dist/js/axiom01.min.js` and `dist/css/axiom01.min.css` as its runtime contract.
- These files must stay byte-for-byte aligned with repository-root `js/axiom01.min.js` and `css/axiom01.min.css`.
...
```text
web/themes/custom/axiom01
```

Then enable it in Appearance settings and clear caches.

## Theme settings (Appearance → Settings → Axiom01)
