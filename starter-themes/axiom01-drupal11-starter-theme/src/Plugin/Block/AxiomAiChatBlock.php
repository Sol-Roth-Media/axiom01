...
/**
 * Provides an Axiom AI chat block.
 *
 * @Block(
 *   id = "axiom01_ai_chat_block",
 *   admin_label = @Translation("Axiom AI Chat"),
...
final class AxiomAiChatBlock extends BlockBase {
  ...
  public function build(): array {
    $id = Html::getUniqueId('axiom-ai-chat');
    return [
      '#type' => 'inline_template',
      '#template' => '<section data-axiom-ai-chat data-provider-mode="{{ provider_mode }}" data-endpoint="{{ endpoint }}" data-system-prompt="{{ system_prompt_preset }}" aria-label="{{ "AI chat"|t }}">{% if intro_text %}<p>{{ intro_text }}</p>{% endif %}<ol data-axiom-ai-chat-messages role="log" aria-live="polite" aria-label="{{ "Chat messages"|t }}"></ol><form data-axiom-ai-chat-form><label for="{{ id }}">{{ "Message"|t }}</label><input id="{{ id }}" name="message" type="text" placeholder="{{ placeholder }}" required /><button type="submit">{{ "Send"|t }}</button></form><p data-axiom-ai-chat-status role="status" aria-live="polite"></p>{% if privacy_notice %}<p>{{ privacy_notice }}</p>{% endif %}</section>',
      '#context' => [
        'id' => $id,
